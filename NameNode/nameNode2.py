    ...
    for assignment in block_assignments:
        ...
        for node in inventory['datanodes']:
            if node['address'] == assigned_datanode:
                # Verificar si el archivo ya está en el datanode
                if file_name in node['files']:
                    node['files'][file_name]['blocks'].append({"name": block_name, "url": block_url})
                else:
                    # Crear el archivo en el datanode si no existe
                    node['files'][file_name] = {"blocks": [{"name": block_name, "url": block_url}]}
